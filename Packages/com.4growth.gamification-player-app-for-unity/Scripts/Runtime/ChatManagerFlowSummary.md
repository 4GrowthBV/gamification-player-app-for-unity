...
| **User Activity Flow** | • Handle user activity notifications from other modules/pop-ups.<br>• Log activities as special `user_activity` messages with structured metadata using enum-based keys.<br>• Auto-populate essential context (AI character name, user name, conversation date) for consistent metadata.<br>• Store activity metadata as JSON in conversation history.<br>• Use AI service and RAG service for intelligent agent selection and context retrieval.<br>• Generate contextual AI responses based on activity completion with enhanced knowledge.<br>• Update profile after activity-triggered conversations.<br>• Trigger `OnAIMessageReceived` with contextual response. | • Track or monitor user activities in other modules.<br>• Generate activity completion notifications.<br>• Define what activities should trigger responses.<br>• Store or manage user metadata directly. | Seamlessly integrates module activities into conversation context with structured metadata system. Activities become part of conversation history for enhanced AI character awareness. |
...
| **AI Flow** | • Detect transition when user sends text.<br>• Add user message to history and backend.<br>• Determine flow based on last message having buttons or not.<br>• Use AI service to select optimal agent based on conversation context.<br>• Use RAG service to retrieve relevant knowledge and examples for selected agent.<br>• Update profile using AI service with structured conversation history.<br>• Request instructions for that agent from cached database using enum-based keys.<br>• Generate response using AI service with enhanced context, examples, and knowledge.<br>• Save both user and AI messages.<br>• Trigger `OnAIMessageReceived` with ChatMessage object. | • Implement AI or LLM logic.<br>• Store or define instructions.<br>• Manage long-term knowledge bases.<br>• Handle document retrieval or embedding storage. | Orchestrates sophisticated agent selection and knowledge retrieval. Uses structured ChatMessage objects and enum-based metadata for type-safe data exchange. |
...
- **Structured metadata system** - enum-based MetadataKeys, Instructions, and PredefinedMessageIdentifiers ensure type-safe, consistent metadata handling.
- **Knowledge-enhanced AI responses** - RAG service integration provides agents with relevant examples and knowledge for superior response quality.
...
- **AI Character Information**: Name and identity of the AI character for consistent persona
...
- **Structured Keys**: Uses enum-based metadata keys for type-safe context handling
- **Conversion Support**: Integration with activity-based message processing

### **Public API Interface**
- **Initialize Chat**: Setup with AI service, RAG service, optional resume context and initial user metadata including AI character information
- **Handle User Messages**: Process free-text user input through enhanced AI flow with agent selection and knowledge retrieval
- **Handle Button Clicks**: Process structured predefined flow interactions with standardized metadata
- **Handle User Activities**: Process cross-module activity notifications with auto-populated structured metadata
- **Get Conversation History**: Retrieve complete conversation as structured message list with rich metadata
- **Force New Conversation**: Reset conversation and profile with optional initial context including AI character setup
...
| **AI Service** | Handles agent selection, AI response generation, and profile updates with structured ChatMessage arrays. Single service for all AI operations with rich context and AI character awareness. |
| **RAG Service** | Provides knowledge retrieval and context enhancement for selected AI agents using conversation history and structured metadata. |
| **UI Layer** | Subscribes to ChatManager events to receive rich ChatMessage objects with buttons, timestamps, streaming support, and structured metadata. |
