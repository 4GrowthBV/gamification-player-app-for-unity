...
namespace GamificationPlayer
{
    public partial class GamificationPlayerEndpoints
    {
        ...
        private IEnumerator CoGetDeviceFlow(Guid deviceFlowId, GetDeviceFlowCallback onReady)
        {
            ...
            if(environmentConfig.TryGetMockDTO<GetDeviceFlowResponseDTO>(out var dto))
            {
                dto.data.attributes.is_validated = true;
                sessionData.AddToLog(dto.data, false);
                onReady?.Invoke(UnityWebRequest.Result.Success, true, dto.data.attributes.user_id);
            } else
            {
