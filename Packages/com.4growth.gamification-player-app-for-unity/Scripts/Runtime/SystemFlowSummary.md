...
| **1a. User Activity Integration** | User completes activity in other module (video, survey, exercise) and returns to chat | • Other module calls `HandleUserActivity()` with rich structured metadata.<br>• Create special `user_activity` message with standardized metadata keys and activity details.<br>• Auto-populate essential context (AI character name, user name, conversation date) using enum-based metadata system.<br>• Store activity metadata as JSON in conversation history.<br>• Use AI service and RAG service for intelligent agent selection and context retrieval.<br>• Generate contextual AI response acknowledging the activity with enhanced knowledge.<br>• Update user profile with activity completion context.<br>• Emit `OnAIMessageReceived` with contextual response. | User sees intelligent, contextual acknowledgment of their recent activity. Chat feels aware of their journey across the app, creating seamless integration between modules with enhanced AI character awareness. |
...
| **3. AI Flow (Triggered by Free Text)** | User types and sends text in the input field | • Add user message to local history + backend as ChatMessage.<br>• Switch to AI flow (determined by message button presence).<br>• Use **AI service** to determine optimal agent for response based on conversation context.<br>• Use **RAG service** to retrieve relevant knowledge and examples for the selected agent.<br>• Use **AI service** to update the user profile with structured conversation history.<br>• Fetch **instructions** for that agent from cached database using structured enum-based keys.<br>• Generate AI response via AI service with enhanced context, examples, and knowledge.<br>• Add AI message to history + backend as ChatMessage.<br>• Emit OnAIMessageReceived with full ChatMessage object.<br>• Update the profile again after AI response using AI service. | The user sees an intelligent, context-aware reply with enhanced knowledge retrieval and potential streaming. The conversation becomes natural and continuous with sophisticated AI agent selection and knowledge-enhanced responses. |
...
- **Structured metadata system** - enum-based metadata keys ensure consistent, type-safe handling of conversation context and AI character information.
- **Knowledge-enhanced responses** - RAG service integration provides AI agents with relevant examples and knowledge for more intelligent responses.
...
|------------|----------------|
| **Chat Manager (client)** | Controls state, flow, and API orchestration with enhanced caching and parallel execution. Handles user activity integration from other modules with structured metadata system. |
...
| **AI Service** | Handles agent selection, AI response generation, and profile updates from structured conversation context with streaming support. Responds contextually to user activities with AI character awareness. |
| **RAG Service** | Retrieves relevant knowledge, examples, and context for selected AI agents to enhance response quality and accuracy. |
| **UI Layer** | Displays rich ChatMessage objects with structured buttons, timestamps, and reacts to enhanced Chat Manager events. |
| **Other Modules (Videos, Surveys, etc.)** | Call `HandleUserActivity()` when users complete activities, seamlessly integrating with chat conversation flow using structured metadata. |
