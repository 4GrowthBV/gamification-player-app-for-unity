...
{
    public class UpdateChatProfileResponseDTOTest
    {
        [Test]
        public void TestDTO()
        {
            ...
            gamificationPlayerEnvironmentConfig.TryGetMockDTO<UpdateChatProfileResponseDTO>(out var dto);
            ...
            Assert.AreEqual(dto.data.Type, "chat_profile");

            Assert.NotNull(dto.data.attributes);

            Assert.That(!string.IsNullOrEmpty(dto.data.attributes.profile));
